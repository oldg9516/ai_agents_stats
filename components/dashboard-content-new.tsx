...
import { useDashboardDataNew } from '@/lib/hooks/use-dashboard-data-new'
...
export function DashboardContentNew() {
	...
	// Fetch dashboard data with React Query (NEW logic)
	const { kpiData, qualityTrends, categoryDistribution, versionComparison, isLoading, error } =
		useDashboardDataNew(filters)
	...
	const handleFiltersChange = (updates: Partial<typeof filters>) => {
		...
		queryClient.invalidateQueries({ queryKey: ['kpi-data-new'] })
		queryClient.invalidateQueries({ queryKey: ['quality-trends-new'] })
		queryClient.invalidateQueries({ queryKey: ['category-distribution-new'] })
		queryClient.invalidateQueries({ queryKey: ['version-comparison-new'] })
	...
	return (
		<div className='flex flex-col gap-4 py-4 md:gap-6 md:py-6 px-4 lg:px-6'>
			...
			{/* KPI Cards Section - NEW with AI Success/Failure rates */}
			{kpiData && <KPISectionNew data={kpiData} />}

			{/* Quality Trends Chart - NEW with success/failure breakdown */}
			{qualityTrends && qualityTrends.length > 0 && (
				<QualityTrendsChart data={qualityTrends} />
			)}

			{/* Category & Version Charts - Side by Side - NEW with success/failure metrics */}
			<div className='grid grid-cols-1 gap-4 md:gap-6 lg:grid-cols-2 overflow-hidden'>
				...
				{versionComparison && versionComparison.length > 0 && (
					<VersionBarChart data={versionComparison} />
				)}
			</div>
