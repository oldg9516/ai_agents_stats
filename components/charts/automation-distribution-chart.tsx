 ...
 */
export const AutomationDistributionChart = memo(function AutomationDistributionChart({
	data,
								...
								return (
									<div className='rounded-lg border bg-background p-3 shadow-md'>
										...
										<div className='text-sm space-y-0.5'>
											...
											<div>
												<span className='inline-block w-2 h-2 rounded-full mr-1.5' style={{ backgroundColor: 'var(--chart-2)' }} />
												<span className='text-muted-foreground'>{t('autoReply')}: </span>
												<span className='font-medium'>{item.autoReplyCount}</span>
												<span className='text-muted-foreground ml-1'>({total > 0 ? ((item.autoReplyCount / total) * 100).toFixed(0) : 0}%)</span>
											</div>
											<div>
												<span className='inline-block w-2 h-2 rounded-full mr-1.5' style={{ backgroundColor: 'var(--chart-4)' }} />
												<span className='text-muted-foreground'>{t('draft')}: </span>
												<span className='font-medium'>{item.draftCount}</span>
												<span className='text-muted-foreground ml-1'>({total > 0 ? ((item.draftCount / total) * 100).toFixed(0) : 0}%)</span>
											</div>
										</div>
									</div>
								)
