 ...
 * Connects to the Dash agent backend via AG-UI protocol.
 * Registers generative UI via useRenderToolCall so backend tool calls render charts.
 */
...
/**
 * Standalone chart renderer for Dash generative UI.
 * Renders Recharts charts inline in the CopilotKit chat.
 */
function DashChart({
	data,
	chartType,
...
/**
 * Inner component that registers CopilotKit generative UI renderers.
 * Must be rendered as a child of <CopilotKit>.
 */
function DashChatActions({ labels }: { labels: { title: string; initial: string; placeholder: string } }) {
	useRenderToolCall({
		name: 'render_chart',
...
export function DashChat({ toggleSlot, className = '' }: DashChatProps) {
	const t = useTranslations('chat.dash')
